...
def predict_threat_probability(input_subject_id, threat_zone=1):
    ...
    current_model_path = MODEL_PATH_FULL + model_name + ".tflearn"
    print ('current path {0}'.format(current_model_path))
    model.load(current_model_path)
    
    # get train and test batches
    #val_features, val_lables = input_pipeline(input_aps_file, PREDICTION_DATA_FOLDER)
