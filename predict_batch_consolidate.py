...
#
#-------------------------------------------------------------------------------------
MIN_POSITIVES_FOR_THREAT = 1
MIN_THRESHOLD_FOR_POSITIVE = 0.85
...
def predict_threat_probability(input_subject_id, threat_zone, model):
    ...
    print (predictions)
    isThreat = process_raw_prediction(predictions)
    print ("overall prediction input:{} zone:{} threat:{}".format(input_subject_id, threat_zone, isThreat)) 
    

def process_raw_prediction(predictions=[]):
    positive_predictions = [prediction for prediction in predictions
                            if prediction[1] > MIN_THRESHOLD_FOR_POSITIVE]
    if len(positive_predictions) >= MIN_POSITIVES_FOR_THREAT:
        return True
    return False
